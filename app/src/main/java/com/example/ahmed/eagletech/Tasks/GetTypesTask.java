...
public class GetTypesTask extends AsyncTask<String, Void, ArrayList> {

    private final String LOG_TAG = UserDataTask.class.getSimpleName();
    private Context mContext;
    public GetTypesTask(Context context){
        mContext = context;
    }
    ...
    protected ArrayList doInBackground(String... params) {
        ...
        try {
            ...
            final String BASE_URL =mContext.getString(R.string.server)+"tasks/manage/get-types?";
    ...
    private ArrayList fetchJSONFromToken(String returnedToken) throws JSONException {
        ...
        JSONArray typesArray =new JSONArray(returnedToken);
        for(int i=0;i<typesArray.length();i++){
            JSONObject typesJSON =typesArray.getJSONObject(i);
            Types type =new Types();
            type.setId(typesJSON.getString("type_id"));
            type.setName(typesJSON.getString("type_name"));
            type.setDescription(typesJSON.getString("type_description"));
            types.add(type);
