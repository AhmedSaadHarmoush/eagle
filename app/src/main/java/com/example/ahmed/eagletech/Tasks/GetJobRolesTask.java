...
public class GetJobRolesTask extends AsyncTask<String, Void, ArrayList> {

    private final String LOG_TAG = UserDataTask.class.getSimpleName();
    private Context mContext;
    public GetJobRolesTask(Context context){
        mContext = context;
    }
    ...
    protected ArrayList doInBackground(String... params) {
        ...
        try {
            ...
            final String BASE_URL =mContext.getString(R.string.server)+"users/job-roles?";
    ...
    private ArrayList fetchJSONFromToken(String returnedToken) throws JSONException {
        ArrayList jobroles = new ArrayList();
        JSONArray jobroleArray =new JSONArray(returnedToken);
        for(int i=0;i<jobroleArray.length();i++){
            JSONObject sevJSON =jobroleArray.getJSONObject(i);
            JobRole jobrole =new JobRole();
            jobrole.setJob_role_id(sevJSON.getString("job_role_id"));
            jobrole.setJob_role_name(sevJSON.getString("job_role_name"));
            jobrole.setJob_role_description(sevJSON.getString("job_role_description"));
